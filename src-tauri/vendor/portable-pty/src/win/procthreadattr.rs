...
impl ProcThreadAttributeList {
    ...
    pub fn set_job(&mut self, job: &mut usize) -> Result<(), Error> {
        // PROC_THREAD_ATTRIBUTE_JOB_LIST from the Windows SDK.
        let result = unsafe {
            UpdateProcThreadAttribute(self.as_mut_ptr(), 0, 0x0002000d,
                job as *mut usize as *mut _, mem::size_of::<usize>(),
                ptr::null_mut(), ptr::null_mut())
        };
        ensure!(result != 0, "assign job attribute: {}", IoError::last_os_error());
        Ok(())
    }
}

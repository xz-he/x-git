...
pub struct CommandBuilder {
    ...
    #[cfg(windows)]
    pub(crate) windows_job: Option<usize>,
}
...
impl CommandBuilder {
    ...
    /// Set the job atomically assigned when the Windows child is created.
    /// # Safety
    /// The handle must name a live Job Object until spawn_command returns.
    #[cfg(windows)]
    pub unsafe fn set_windows_job(&mut self, handle: usize) {
        self.windows_job = Some(handle);
    }

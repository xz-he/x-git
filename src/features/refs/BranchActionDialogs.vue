...
const integrationTarget = computed(() => refs.integrationTargets.find(
  (branch) => branch.fullName === refs.integrationRequest?.target,
));
...
<template>
  <div v-if="props.branch" class="branch-actions" aria-label="分支操作" :inert="repositories.navigationBusy || undefined">
    ...
    <template v-if="isMutableTarget">
      ...
      <button :aria-label="`合并分支 ${props.branch.name}`" title="合并分支" :disabled="refs.integrationBlocked" @click="open('merge')"><GitMerge :size="15" />合并</button>
  ...
  <ConfirmDialog v-if="refs.integrationRequest" :title="refs.integrationRequest.action === 'merge' ? '合并分支' : '变基当前分支'" :description="refs.integrationRequest.action === 'merge' ? `将 ${integrationTarget?.name ?? '所选目标分支'} 合并到 ${refs.integrationRequest.source}。` : `将 ${refs.integrationRequest.source} 变基到 ${integrationTarget?.name ?? '所选目标分支'}。`" :confirm-label="refs.integrationRequest.action === 'merge' ? '合并分支' : '开始变基'" :confirm-disabled="refs.integrationBlocked || refs.loading || !integrationTarget" :busy="refs.submitting" @cancel="refs.cancelIntegration" @confirm="refs.confirmIntegration">
    ...
    <p v-if="!refs.loading && refs.snapshot && !refs.integrationTargets.length">没有可用的目标分支</p>
